            ...
            if (session.getAttribute("user_type") != null) {
                if (session.getAttribute("user_type").toString().equalsIgnoreCase("doctor")) {
                    ...
                    if (request.getParameter("changePass") != null) {
                        ...
                        if (old == "" || npas == "" || cpas == "") {
                            response.sendRedirect("ChangePass?errMsg=All the fields are required !");
                        } else {
                            DBConnection db = new DBConnection();
                            db.pstmt = db.con.prepareStatement("SELECT pass FROM doctor_reg WHERE did = ?");
                            db.pstmt.setString(1, session.getAttribute("user_id").toString());
                            db.rst = db.pstmt.executeQuery();
                            if (db.rst.next()) {
                                if (old.equals(db.rst.getString(1))) {
                                    if (npas.equals(cpas)) {
                                        db.pstmt = db.con.prepareStatement("UPDATE doctor_reg SET pass = ? WHERE did = ?");
                                        ...
                                        if (done > 0) {
                                            ...
                                            db.pstmt.setString(1, npas);
                                            db.pstmt.setString(2, session.getAttribute("user_id").toString());
                                            db.pstmt.setString(3, "Doctor");
                                            int done_all = db.pstmt.executeUpdate();
                                            if (done_all > 0) {
                                                response.sendRedirect("ChangePass?succMsg=Successfully changed your password !");
                                            } else {
                                                response.sendRedirect("ChangePass?errMsg=Something went wrong !");
                                            }
                                        } else {
                                            response.sendRedirect("ChangePass?errMsg=Something went wrong !");
                                        }
                                    } else {
                                        response.sendRedirect("ChangePass?errMsg=New password and confirm password does not matched!");
                                    }
                                } else {
                                    response.sendRedirect("ChangePass?errMsg=Old Password is incorrect !");
                                }
                            }
                        }
                    }
