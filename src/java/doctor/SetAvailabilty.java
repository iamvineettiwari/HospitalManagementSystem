 ...
 */
public class SetAvailabilty extends HttpServlet {
            ...
            if (session.getAttribute("user_type") != null) {
                if (session.getAttribute("user_type").toString().equalsIgnoreCase("doctor")) {
                    ...
                    out.println("<div class=\"row\">"
                            ...
                            + "<h6 class=\"m-0 font-weight-bold text-primary\">Set Availabilty</h6>"
                            + "</div>"
                            ...
                            + "</div>"
                            + "<div class=\"col-md-4\"><input type=\"date\" name=\"date\" class=\"form-control user-control\"></div>"
                            + "<div class=\"col-md-4\"><input type=\"submit\" name=\"saveAvail\" value=\"Not Available\" class=\"btn btn-danger btn-block btn-user\"></div>"
                            + "</div>"
                            + "</div>"
                            + "</div>"
                            + "</div>"
                    ...
                    if (request.getParameter("saveAvail") != null) {
                        if (request.getParameter("date") == "") {
                            response.sendRedirect("SetAvailabilty?errMsg=Please select a date !");
                        } else {
                            DBConnection db = new DBConnection();
                            db.pstmt = db.con.prepareStatement("INSERT INTO doctor_availabilty (did, date) VALUES (?,?)");
                            db.pstmt.setString(1, session.getAttribute("user_id").toString());
                            db.pstmt.setString(2, request.getParameter("date"));
                            int done = db.pstmt.executeUpdate();
                            if (done > 0) {
                                response.sendRedirect("SetAvailabilty?succMsg=Successfully update your available dates");
                            } else {
                                response.sendRedirect("SetAvailabilty?errMsg=Something went wrong while updating availabilty !");
                            }
                        }
                    }
                } else {
