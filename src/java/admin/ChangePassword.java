            ...
            if (session.getAttribute("user_type") != null) {
                if (session.getAttribute("user_type").toString().equalsIgnoreCase("admin")) {
                    ...
                    if (request.getParameter("changePass") != null) {
                        ...
                        if (old == "" || npas == "" || cpas == "") {
                            response.sendRedirect("ChangePassword?errMsg=All the fields are required !");
                        } else {
                            DBConnection db = new DBConnection();
                            db.pstmt = db.con.prepareStatement("SELECT admin_pass FROM admin_reg WHERE admin_id = ?");
                            db.pstmt.setString(1, session.getAttribute("user_id").toString());
                            db.rst = db.pstmt.executeQuery();
                            if (db.rst.next()) {
                                if (old.equals(db.rst.getString(1))) {
                                    if (npas.equals(cpas)) {
                                        db.pstmt = db.con.prepareStatement("UPDATE admin_reg SET admin_pass = ? WHERE admin_id = ?");
                                        ...
                                        if (done > 0) {
                                            ...
                                            if (done_all > 0) {
                                                response.sendRedirect("ChangePassword?succMsg=Successfully changed your password !");
                                            } else {
                                                response.sendRedirect("ChangePassword?errMsg=Something went wrong !");
                                            }
                                        } else {
                                            response.sendRedirect("ChangePassword?errMsg=Something went wrong !");
                                        }
                                    } else {
                                        response.sendRedirect("ChangePassword?errMsg=New password and confirm password does not matched!");
                                    }
                                } else {
                                    response.sendRedirect("ChangePassword?errMsg=Old Password is incorrect !");
                                }
                            }
                        }
                    }
