        ...
        try {
            ...
            if (session.getAttribute("user_type") != null) {
                if (session.getAttribute("user_type").toString().equalsIgnoreCase("admin")) {
                    ...
                    db.pstmt = db.con.prepareStatement("SELECT * FROM doctor_reg");
                    db.rst = db.pstmt.executeQuery();
                    while (db.rst.next()) {
                        out.println("<tr>"
                                + "<form action=\"EditDoctor\" method=\"post\"><input type='hidden' name='docid' value='" + db.rst.getString(1) + "'>"
                                ...
                                + "<td><button type=\"submit\" name=\"edit\" class=\"btn btn-primary btn-user\"><i class=\"fas fa-pen\"></i></button></td>"
